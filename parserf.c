...
Token* generate_operation_nodes(Token* current_token, Node* current_node) {
    ...
    if (current_token->type == INT) {
        ...
        identifier_node =
            init_node(identifier_node, current_token->value, IDENTIFIER);
    ...
    current_token++;
    current_token++;
    while (current_token->type == INT || current_token->type == IDENTIFIER ||
        ...
        if (current_token->type == INT || current_token->type == IDENTIFIER) {
            ...
            if (current_token->type != OPERATOR) {
                ...
                if (current_token->type == INT) {
                    ...
                    second_expr_node =
                        init_node(second_expr_node, current_token->value, INT);
                    current_node->right = second_expr_node;
                } else if (current_token->type == IDENTIFIER) {
                    Node* second_identifier_node = malloc(sizeof(Node));
                    second_identifier_node =
                        init_node(second_identifier_node, current_token->value,
                                  IDENTIFIER);
        ...
        if (current_token->type == OPERATOR) {
            ...
            next_oper_node =
                init_node(next_oper_node, current_token->value, OPERATOR);
            ...
            if (current_token->type == INT) {
                ...
                Node* second_identifier_node = malloc(sizeof(Node));
                second_identifier_node = init_node(
                    second_identifier_node, current_token->value, IDENTIFIER);
...
Node* handle_exit_syscall(Node* root, Token* current_token, Node* current) {
    ...
    if (strcmp(current_token->value, "(") == 0 &&
        ...
        open_paren_node =
            init_node(open_paren_node, current_token->value, SEPARATOR);
        ...
        if (current_token->type == INT || current_token->type == IDENTIFIER) {
            ...
            if (current_token->type == OPERATOR && current_token != NULL) {
                ...
                current_token--;
                Node* expr_node = malloc(sizeof(Node));
                expr_node = init_node(expr_node, current_token->value,
                                      current_token->type);
            ...
            if (strcmp(current_token->value, ")") == 0 &&
                current_token->type == SEPARATOR &&
                current_token->type != END_OF_TOKENS) {
                Node* close_paren_node = malloc(sizeof(Node));
                close_paren_node = init_node(close_paren_node,
                                             current_token->value, SEPARATOR);
                ...
                if (strcmp(current_token->value, ";") == 0 &&
                    ...
                    semi_node =
                        init_node(semi_node, current_token->value, SEPARATOR);
...
void handle_token_errors(char* error_text, Token* current_token,
                         TokenType type) {
...
Node* create_variable_reusage(Token* current_token, Node* current) {
    ...
    main_identifier_node =
        init_node(main_identifier_node, current_token->value, IDENTIFIER);
    current->left = main_identifier_node;
    current = main_identifier_node;
    current_token++;

    handle_token_errors("Invalid syntax after idenfitier", current_token,
                        OPERATOR);
    ...
    if (current_token->type == END_OF_TOKENS ||
        (current_token->type != INT && current_token->type != IDENTIFIER)) {
        ...
        if (current_token->type == OPERATOR) {
            ...
            while (operation) {
                ...
                if (current_token->type == INT) {
                    ...
                    Node* identifier_node = malloc(sizeof(Node));
                    identifier_node = init_node(
                        identifier_node, current_token->value, IDENTIFIER);
                ...
                if (current_token->type == OPERATOR) {
                    ...
                    if (current_token->type != OPERATOR) {
                        ...
                        if (current_token->type == INT) {
                            Node* expr_node = malloc(sizeof(Node));
                            expr_node =
                                init_node(expr_node, current_token->value, INT);
                            current->right = expr_node;
                            current_token++;
                        } else if (current_token->type == IDENTIFIER) {
                            Node* identifier_node = malloc(sizeof(Node));
                            identifier_node =
                                init_node(identifier_node, current_token->value,
                                          IDENTIFIER);
                        ...
                        current_token--;
                        Node* oper_node = malloc(sizeof(Node));
                        oper_node = init_node(oper_node, current_token->value,
                                              OPERATOR);
                        current->right = oper_node;
                        current = oper_node;
                    }
            ...
            if (current_token->type == INT) {
                ...
                Node* identifier_node = malloc(sizeof(Node));
                identifier_node = init_node(identifier_node,
    ...
    handle_token_errors("Invalid Syntax After Expression", current_token,
                        SEPARATOR);
...
Node* create_variables(Token* current_token, Node* current) {
    ...
    handle_token_errors("Invalid Syntax After Identifier", current_token,
                        OPERATOR);
...
Token* generate_if_operation_nodes_right(Token* current_token,
                                         Node* current_node) {
    Node* oper_node = malloc(sizeof(Node));
    oper_node = init_node(oper_node, current_token->value, OPERATOR);
...
Node* create_if_statement(Token* current_token, Node* current) {
    ...
    if (current_token->type != OPERATOR || current_token->type == COMP) {
        ...
        Node* expr_node = malloc(sizeof(Node));
        expr_node =
            init_node(expr_node, current_token->value, current_token->type);
    ...
    if (current_token->type == SEPARATOR) {
        ...
        current_token =
            generate_if_operation_nodes_right(current_token, current);
    }
...
Node* handle_write_node(Token* current_token, Node* current) {
    ...
    write_node =
        init_node(write_node, current_token->value, current_token->type);
    ...
    if (current_token->type != STRING && current_token->type != IDENTIFIER) {
        handle_token_errors("ERROR: Expected String Literal", current_token,
                            STRING);
    }

    Node* string_node = NULL;
    string_node =
        init_node(string_node, current_token->value, current_token->type);
    ...
    number_node =
        init_node(number_node, current_token->value, current_token->type);
        ...
        switch (current_token->type) {
            ...
            case SEPARATOR:
                ...
                if (strcmp(current_token->value, "}") == 0) {
                    ...
                    close_curly = init_node(close_curly, current_token->value,
                                            current_token->type);
